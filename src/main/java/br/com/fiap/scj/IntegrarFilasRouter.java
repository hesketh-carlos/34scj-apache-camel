...
public class IntegrarFilasRouter extends RouteBuilder {
	...
	public void configure() throws Exception {
		from(endpointInput).tracing().log("input >>> ${body}")
		...
		.to(endpointOutput).log("output >>> ${body}")
		.doCatch(RuntimeException.class).setBody(constant("${body}")).log("Message to be sent: ${body}")
		.log("Erro!!")
		.end();
